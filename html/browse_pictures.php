...
if (isset($_REQUEST['user_id'])) {
    ...
    $sql = "SELECT * FROM data WHERE user_id = '$user' AND type='picture' ORDER BY data_id DESC";

    echo '<div class="w-container" style="border-bottom: solid; border-bottom-width: thick;">
        <h1>My Pictures</h1>
    </div>';
} else {
    $sql = "SELECT * FROM data WHERE type='picture' ORDER BY data_id DESC";
    echo '<div class="w-container" style="border-bottom: solid; border-bottom-width: thick;">
        <h1>Pictures</h1>
    </div>';
}
...
if ($resultData = mysqli_query($conn, $sql)) {
    ...
    if($resultData->num_rows != 0) {
        ...
        echo '<div class="w-container">
                    <h4>There are no pictures to browse.</h4>
                </div>';
    }
